...
watch(
    ...
    (newImages) => {
        if (newImages?.length > 0) {
            mainImgUrl.value = newImages[0]?.url;
        }
    },
    { immediate: true },
);
...
<template>
    <div>
        ...
        <NuxtImg class="rounded-xl object-contain w-full min-w-[350px]" :width="imgWidth" :height="imgWidth" alt="product-name" title="product-name" :src="mainImgUrl" fetchpriority="high" placeholder placeholder-class="blur-xl" />
