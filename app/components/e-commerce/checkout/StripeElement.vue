...
async function createPayment() {
    ...
    const { clientSecret, error, message } = await $fetch("/api/payment/create-payment", {
        method: "post",
        body: JSON.stringify({
...
<template>
    <div>
        ...
        <button v-if="userCookie" @click="createPayment" :disabled="isLoading" class="flex items-center justify-center gap-3 p-3 mt-4 font-semibold text-center text-white rounded-lg shadow-md bg-primary hover:bg-primary-dark disabled:cursor-not-allowed disabled:bg-gray-400">
        ...
        <div v-else>
            <p class="mt-1 text-gray-500">
                ...
                <NuxtLink href="/auth/signin" class="text-bold text-red-500">Sign in</NuxtLink>
                .
            </p>
        </div>
    </div>
