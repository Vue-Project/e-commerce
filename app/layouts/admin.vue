...
<template>
    <div class="flex h-screen bg-gray-100">
        ...
        <aside :class="['bg-white shadow-lg h-full transition-all duration-300 flex flex-col', drawerOpen ? 'w-59' : 'w-16']">
            <div class="flex items-center justify-between px-4 py-3 border-b">
                <span v-if="drawerOpen" class="text-xl font-bold">Dashboard</span>
                <button @click="toggleDrawer" class="p-2 rounded-full hover:bg-gray-200 focus:outline-none focus:ring">
            ...
            <nav class="flex-1 mt-4 space-y-2">
                <nuxtLink v-for="menu in links" :key="menu" :to="menu.link" class="flex items-center gap-4 px-4 py-2 text-gray-700 hover:bg-gray-100">
