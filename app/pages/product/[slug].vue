    ...
    <main class="container relative py-6 xl:max-w-7xl">
        <div>
            ...
            <div class="flex flex-col gap-10 md:flex-row md:justify-between lg:gap-24">
                <ProductImageGallery :images="singleProductData?.products?.images" class="relative flex-1" />
              ...
              /> -->
                    ...
                    <div class="fixed bottom-0 left-0 z-10 flex items-center w-full gap-4 p-4 mt-12 bg-white md:static md:bg-transparent bg-opacity-90 md:p-0">
                        <input v-model="defaultQuantity" type="number" min="1" aria-label="Quantity" class="bg-white border rounded-lg flex text-left p-2.5 w-20 gap-4 items-center justify-center focus:outline-none" />
                        <AddToCartButton class="flex-1 w-full md:max-w-xs" @click="shoppingCartStore.addProductToCart(singleProductData?.products)" :disabled="false" :class="{ loading: true }" />
                    </div>
